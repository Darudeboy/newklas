...
class JiraConfluenceSync:
    ...
    def get_release_links_direct(
        ...
        try:
            ...
            if not target_keys:
                logging.warning(
                    f"⚠️ В релизе не найдено связей 'consists of' для проекта {self.target_project}"
                )
                return []

            logging.info(
                f"🔗 Найдено ключей: {len(target_keys)}. Запрашиваем детали и описания..."
            )
    ...
    def generate_content(
        self, release_key: str, issues: List[Tuple[str, str, str, str]]
    ) -> str:
        rows = []
        for key, summary, issue_type, _ in issues:
            ...
            macro = (
                ...
                f'<ac:parameter ac:name="showSummary">true</ac:parameter>'
                f"</ac:structured-macro>"
            )
            rows.append(f"<tr><td>{macro}</td><td>{issue_type}</td><td>{safe_summary}</td></tr>")

        table_html = (
            '<table class="wrapped"><thead><tr><th>Задача</th><th>Тип</th><th>Название</th></tr></thead>'
        ...
        for _, summary, _, description in issues:
            ...
            if clean_desc:
                item = (
                    f"<li><strong>{safe_summary}</strong><br/>"
                    f"<span style='color: #5e6c84; font-size: 90%;'>{clean_desc}</span></li>"
                )
            else:
            ...
            if "story" in issue_type.lower() or "история" in issue_type.lower():
                ...
                if clean_desc:
                    stories_items.append(
                        f"<li><strong>{safe_summary}</strong><br/><span style='color: #5e6c84;'>{clean_desc}</span></li>"
                    )
                else:
    ...
    def update_page(self, release_key: str):
        ...
        if existing:
            ...
            self.confluence.update_page(
                existing["id"], title, content, representation="storage", minor_edit=False
            )
            page_id = existing["id"]
        else:
            ...
            new_page = self.confluence.create_page(
                ...
                title,
                content,
                parent_id=self.config["confluence"]["parent_page_id"],
